...
def entropy(rows):
    ...
    for lbl in counts:
        prob_of_lbl = counts[lbl] / len_rows
...
class Leaf:
    ...
    def __init__(self, rows, id, depth):
        ...
        self.id = id
        self.depth = depth
        self.label = list(self.predictions.keys())[\
            list(self.predictions.values()).index(max(list(self.predictions.values())))]
...
def print_tree(node, spacing=""):
    ...
    # Base case: we've reached a leaf
    if isinstance(node, Leaf):
        print(spacing + '(Id:{}  Depth:{}  Label:{})'.format(node.id, node.depth, node.label))
        # print (spacing + "Predict", node.lebel)
        return

    # Print the question at this node
    print (spacing + '(Id:{}  Depth:{}  Question:{})'.format(node.id, node.depth, node.question))
...
def getLeafNodes(node, leafNodes =[]):
    ...
    leftNodes = getLeafNodes(node.true_branch)
    rightNodes = getLeafNodes(node.false_branch)
...
def getInnerNodes(node, innerNodes =[]):
    ...
    leftNodes = getInnerNodes(node.true_branch)
    rightNodes = getInnerNodes(node.false_branch)
...
def computeAccuracy(rows, node):
    ...
    return round(numAccurate / totalRows, 3)

def sortNodes(nodes, attr):
    idxs_list = [eval('node.{}'.format(attr)) for node in nodes]
    return np.sort(np.array(idxs_list))
